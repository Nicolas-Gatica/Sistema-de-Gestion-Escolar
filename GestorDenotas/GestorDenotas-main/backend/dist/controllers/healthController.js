...
const supabase_1 = require("../config/supabase"); // Importamos Supabase (ya no Prisma)
...
router.get('/database', async (_req, res, next) => {
    try {
        ...
        const { error: queryError } = await supabase_1.supabase.from('Asignatura').select('id', { count: 'exact', head: true }).limit(1);
        if (queryError)
            throw queryError;
        ...
        const [estudiantes, profesores, cursos, calificaciones] = await Promise.all([
            supabase_1.supabase.from('Estudiante').select('id', { count: 'exact', head: true }),
            supabase_1.supabase.from('Profesor').select('id', { count: 'exact', head: true }),
            supabase_1.supabase.from('Curso').select('id', { count: 'exact', head: true }),
            supabase_1.supabase.from('Calificacion').select('id', { count: 'exact', head: true })
        ]);
        res.json({
            status: 'CONNECTED',
