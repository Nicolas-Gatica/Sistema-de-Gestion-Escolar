...
router.get("/", async (_req, res, next) => {
    try {
        ...
        const { data: estudiantes, error } = await supabase_1.supabase
            .from('Estudiante')
            .select(`
            id,
            user_uuid,
 ...
 */
router.get("/curso/:cursoId", async (req, res, next) => {
    try {
...
router.post("/", async (req, res, next) => {
    try {
        ...
        if (!email) {
            throw errors_1.createError.badRequest("El email es obligatorio para crear un estudiante.");
        }
        ...
        const { data: nuevoEstudiante, error } = await supabase_1.supabase
            .from('Estudiante')
            .insert([{
...
router.get("/:id/calificaciones", async (req, res, next) => {
    try {
        ...
        const { data: estData } = await supabase_1.supabase
            .from('Estudiante')
            .select('user_uuid')
            .eq('id', estudianteId)
            .single();
...
router.get("/:id", async (req, res, next) => {
    try {
        ...
        const { data: estudiante, error } = await supabase_1.supabase
            .from('Estudiante')
            .select(`
            *,
            Curso ( nombre ),
...
router.put("/:id", async (req, res, next) => {
    try {
        ...
        const { data: estudianteActualizado, error } = await supabase_1.supabase
            .from('Estudiante')
            .update({
            nombre,
            apellido,
...
router.delete("/:id", async (req, res, next) => {
    try {
        ...
        const { error: authError } = await supabase_1.supabase.auth.admin.deleteUser(estData.user_uuid);
        if (authError)
            throw authError;
        res.status(204).send();
    }
